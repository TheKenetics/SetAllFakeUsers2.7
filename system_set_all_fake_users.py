bl_info = {
	...
	"warning": "Doesn't update UI when run, to update UI manually, move mouse over fake user button",
	"wiki_url": "",
	"category": "System"
}
...
class RAFU_OT_SetAllFakeUsers(bpy.types.Operator):
	...
	set_lamp = BoolProperty(
		name="Lamps",
		description="Sets fake users for all lamp data",
		default=True
	)
	...
	def execute(self, context):
		...
		if self.set_lamp:
			set_fake_users_in_data(context.blend_data.lamps, action)
		if self.set_lattice:
...
def register():
	...
	bpy.types.INFO_MT_file_external_data.append(set_fake_users_button)

def unregister():
	bpy.types.INFO_MT_file_external_data.remove(set_fake_users_button)
